...
| `resource_availability_check_max_attempts` | The maximum number of attempts to check if a resource (service account and/or pull secret) set is available before giving up. There is 5 seconds interval between each attempt (default is `5`). [Introduced](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/27664) in GitLab 15.0. Starting with GitLab 15.2, setting it to `0` will disable the resource checks altogether. |
...
Starting with **GitLab 15.2**, it is possible to disable this feature altogether 
by setting the `resource_availability_check_max_attempts` to `0`
