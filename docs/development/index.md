...
wget https://storage.googleapis.com/golang/go1.22.3.linux-amd64.tar.gz
...
```shell
wget https://storage.googleapis.com/golang/go1.22.3.darwin-amd64.tar.gz
sudo tar -C /usr/local -xzf go*-*.tar.gz
export PATH="$(go env GOBIN):$PATH"
```
...
```shell
wget https://storage.googleapis.com/golang/go1.22.3.darwin-amd64.pkg
open go*-*.pkg
export PATH="$(go env GOBIN):$PATH"
```
...
```shell
pkg install go-1.22.3 gmake git mercurial
export PATH="$(go env GOBIN):$PATH"
```
