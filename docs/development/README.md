...
```bash
wget https://storage.googleapis.com/golang/go1.5.4.darwin-amd64.tar.gz
sudo tar -C /usr/local -xzf go*-*.tar.gz
```
...
```
wget https://storage.googleapis.com/golang/go1.5.4.darwin-amd64.pkg
open go*-*.pkg
```

### For FreeBSD
```
pkg install go-1.5.3 gmake git mercurial
```
