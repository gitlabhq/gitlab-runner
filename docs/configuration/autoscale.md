...
**Note**: In GitLab 17.5, we have deprecated the the GitLab Runner Docker Machine executor. We recommend that customers using the Runner Docker Machine executor on Amazon Web Services (AWS) EC2, Microsoft Azure Compute, and Google Compute Engine (GCE) migrate to the [GitLab-developed fleeting and taskscaler autoscaler](../runner_autoscale/index.md).
