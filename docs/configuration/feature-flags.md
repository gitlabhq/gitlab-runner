...
| `FF_USE_FASTZIP` | `true` | {{< icon name="check-circle" >}} Yes | 19.7 | Fastzip is a performant archiver for cache/artifact archiving and extraction. Enabled by default. Set to `false` to use the legacy zip implementation. Deprecated: The legacy zip implementation and this flag are removed in GitLab Runner 19.7, after which fastzip is always used. |
