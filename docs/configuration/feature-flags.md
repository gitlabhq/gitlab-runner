...
| `FF_GIT_URLS_WITHOUT_TOKENS` | `true` | **{dotted-circle}** No |  | When enabled, when setting up the Git configuration, or run Git commands, we do not embed the job token anywhere. Rather, we set up a Git credential helper, which - on demand - uses the environment variable to obtain the job token. This limits where the token is stored and therefore the potential for it to leak. |
