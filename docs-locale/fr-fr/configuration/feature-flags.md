...
| `FF_USE_NEW_BASH_EVAL_STRATEGY` | `false` | {{< icon name="check-circle" >}} Oui |  | Sans effet (no-op) et déprécié. L'appel Bash `eval` s'exécute dans un sous-shell. Cette action remplace la protection d'origine et empêche la fuite du corps du script lors de l'annulation. Pour plus d'informations, consultez [l'issue 39005](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/39005). Pour ne pas utiliser cette option et revenir à la forme d'invocation de pipeline héritée (le comportement lorsque cet indicateur était défini sur `false`), utilisez plutôt le nouvel indicateur `FF_USE_LEGACY_BASH_EVAL`. Cet indicateur sera supprimé dans une future release. |
| `FF_USE_LEGACY_BASH_EVAL` | `false` | {{< icon name="dotted-circle" >}} Non |  | Lorsque défini sur `true`, l'appel Bash `eval` utilise la forme d'invocation de pipeline héritée (le même comportement que le `FF_USE_NEW_BASH_EVAL_STRATEGY=false` déprécié). La valeur par défaut (`false`) encapsule `eval` dans un sous-shell piégé pour éviter les fuites du corps du script lors de l'annulation dans les versions de Bash antérieures à la 4.4 et dans Bash 5.3 et versions ultérieures. Pour plus d'informations, consultez [l'issue 39005](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/39005). Utilisez cet indicateur si le comportement par défaut cause des problèmes. |
...
| `FF_USE_EXPONENTIAL_BACKOFF_STAGE_RETRY` | `true` | {{< icon name="dotted-circle" >}} Non |  | Lorsqu'activé, les nouvelles tentatives pour `GET_SOURCES_ATTEMPTS`, `ARTIFACT_DOWNLOAD_ATTEMPTS`, `RESTORE_CACHE_ATTEMPTS`, `EXECUTOR_JOB_SECTION_ATTEMPTS` et `SECRETS_RETRIEVAL_ATTEMPTS` utilisent un backoff exponentiel (5 sec - 5 min). |
...
| `FF_ENABLE_JOB_INPUTS_INTERPOLATION` | `true` | {{< icon name="dotted-circle" >}} Non |  | Lorsqu'il est activé, les entrées de job sont interpolées. Pour plus d'informations, consultez [&17833](https://gitlab.com/groups/gitlab-org/-/work_items/17833). |
...
| `FF_USE_NATIVE_CONTAINER_STOP` | `false` | {{< icon name="dotted-circle" >}} Non |  | Lorsqu'activé, le mécanisme d'arrêt natif du moteur de conteneurs est utilisé pour terminer le conteneur de build lors de l'annulation d'un job. SIGTERM est envoyé au PID 1 uniquement. Le PID 1 doit arrêter correctement et proprement le reste de l'arborescence des processus. N'activez cet indicateur que lorsque le PID 1 est connu pour gérer SIGTERM et propager l'arrêt à ses descendants (comme un processus init qui transmet les signaux ou une application conçue pour coordonner son propre arrêt). Lorsqu'il est désactivé (par défaut), le runner applique son propre mécanisme d'arrêt progressif, qui envoie des signaux directement aux processus descendants. L'envoi direct de signaux est nécessaire lorsque l'arborescence des processus contient des shells ou d'autres processus qui ne propagent pas SIGTERM à leurs enfants. |
