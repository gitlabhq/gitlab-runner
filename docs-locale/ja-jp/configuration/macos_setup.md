...
- システムShellをBashに変更する。
- Homebrew、rbenv、GitLab Runnerをインストールする。
- rbenvを設定し、Rubyをインストールする。
- Xcodeをインストールする。
- Runnerを登録する。
- CI/CDを設定する。
...
- macOSの最新バージョンをインストールします。このガイドは11.4で作成されました。
- ターミナルまたはSSHでマシンにアクセスできることを確認します。

## システムShellをBashに変更する {#change-the-system-shell-to-bash}

新しいバージョンのmacOSでは、デフォルトのShellとしてZshが使用されます。ただし、RunnerのShell executorでは、CI/CDスクリプトが正しく実行されるようにBashが必要です。多くのスクリプトがBash固有の構文と機能を使用しているためです。
...
## rbenvを設定し、Rubyをインストールする {#configure-rbenv-and-install-ruby}

次に、rbenvを設定し、Rubyをインストールします。

1. Bash環境にrbenvを追加します:
...
1. ライセンスに同意し、推奨される追加コンポーネントをインストールします。これを行うには、Xcodeを開いて表示されたプロンプトに従うか、ターミナルで次のコマンドを実行します:

   ```shell
   sudo xcodebuild -runFirstLaunch
   ```

1. ビルド中にXcodeが適切なコマンドラインツールを読み込むように、アクティブなデベロッパーディレクトリを更新します:
...
プロジェクトRunnerを[作成して登録](https://docs.gitlab.com/ci/runners/runners_scope/#create-a-project-runner-with-a-runner-authentication-token)します。

Runnerを作成して登録する際は、次のようにします:
...
これで、macOS Runnerがプロジェクトをビルドできるようになりました。
