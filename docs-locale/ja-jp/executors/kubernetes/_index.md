...
- 以下のロジックを使用して命名されたサービスコンテナ:
  - サービスに有効なエイリアス（[DNS label name](https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#dns-label-names)）があり、他のサービスコンテナでまだ使用されていない場合、そのエイリアスがコンテナ名として使用されます。
  - 有効なエイリアスがない場合、コンテナは`svc-N`と命名され、`N`は`0`から始まるシーケンシャルインデックスです。

{{< history >}}

- Runner 17.9でエイリアスベースの命名が[導入](https://gitlab.com/gitlab-org/gitlab-runner/-/merge_requests/4469)されました。
...
| `context`   | `kubectl`設定から使用するオプションのKubernetesコンテキスト名。`host`を指定しない場合、このオプションを使用します。 |
...
|----------|-------------------------------|
| apps/deployments | 作成 (`kubernetes.autoscaler`), 削除 (`kubernetes.autoscaler`), 取得 (`kubernetes.autoscaler`), 一覧表示 (`kubernetes.autoscaler`), 更新 (`kubernetes.autoscaler`) |
...
- `pods/log`権限は、次のいずれかのシナリオに該当する場合にのみ必要です:
  - [`FF_KUBERNETES_HONOR_ENTRYPOINT`機能フラグ](../../configuration/feature-flags.md)が有効になっている。
  - [`CI_DEBUG_SERVICES`変数](https://docs.gitlab.com/ci/services/#capturing-service-container-logs)が`true`に設定されている場合に、[`FF_USE_LEGACY_KUBERNETES_EXECUTION_STRATEGY`機能フラグ](../../configuration/feature-flags.md)が無効になっている。
  - [`FF_WAIT_FOR_POD_TO_BE_REACHABLE`機能フラグ](../../configuration/feature-flags.md)が有効になっている。
...
| `pod_cpu_limit`                             | ビルドポッドに対して指定されるCPU割り当て。 |
| `pod_cpu_limit_overwrite_max_allowed`       | ビルドポッドのCPU割り当てを上書きできる最大量。空の場合、CPU制限上書き機能が無効になります。 |
| `pod_cpu_request`                           | ビルドポッドに対してリクエストされるCPU割り当て。 |
| `pod_cpu_request_overwrite_max_allowed`     | ビルドポッドのCPU割り当てリクエストを上書きできる最大量。空の場合、CPUリクエスト上書き機能が無効になります。 |

> [!note]
> ポッドレベルのリソース仕様は、[Kubernetes v1.32](https://v1-32.docs.kubernetes.io/blog/2024/12/11/kubernetes-v1-32-release/#pod-level-resource-specifications)でアルファ機能フラグとして導入され、[Kubernetes v1.34](https://kubernetes.io/blog/2025/09/22/kubernetes-v1-34-pod-level-resources/)でベータに昇格されました。
...
| `pod_memory_limit`                             | ビルドポッドに割り当てられるメモリの量。 |
| `pod_memory_limit_overwrite_max_allowed`       | ビルドポッドのメモリ割り当てを上書きできる最大量。空の場合、メモリ制限上書き機能が無効になります。 |
| `pod_memory_request`                           | ビルドポッドに対してリクエストされるメモリの量。 |
| `pod_memory_request_overwrite_max_allowed`     | ビルドポッドのメモリ割り当てリクエストを上書きできる最大量。空の場合、メモリリクエスト上書き機能が無効になります。 |

#### ヘルパーコンテナのメモリサイズ設定に関する推奨事項 {#helper-container-memory-sizing-recommendations}

最適なパフォーマンスを得るには、ワークロードの要件に基づいてヘルパーコンテナのメモリ制限を設定します:

- **キャッシュおよびアーティファクト生成を実行するワークロード**: 最低250 MiB
- **キャッシュやアーティファクトのない基本的なワークロード**: より低い制限 (128～200 MiB) でも機能する可能性があります
...
| `allowed_groups`                              | コンテナグループに指定できるグループIDの配列。存在しない場合、すべてのグループが許可されます。詳細については、[コンテナユーザーとグループを設定する](#configure-container-user-and-group)を参照してください。 |
...
| `allowed_users`                               | コンテナユーザーに指定できるユーザーIDの配列。存在しない場合、すべてのユーザーが許可されます。詳細については、[コンテナユーザーとグループを設定する](#configure-container-user-and-group)を参照してください。 |
...
| `pod_spec`                                    | これは実験的な設定です。Runnerマネージャーによって生成されるポッド仕様を、CIジョブの実行に使用されるポッドで設定された設定のリストで上書きします。`Kubernetes Pod Specification`にリストされているすべてのプロパティを設定できます。詳細については、[生成されたポッド仕様を上書きする（実験的機能）](#overwrite-generated-pod-specifications)を参照してください。 |
...
| `pod_disruption_budget`                       | 有効にすると、ジョブポッドごとに[`PodDisruptionBudget`](https://kubernetes.io/docs/tasks/run-application/configure-pdb/)が作成され、ノードのドレインやクラスターのアップグレードなど自主的な中断の際にポッドが削除されるのを防ぎます。デフォルトでは無効になっています。[`poddisruptionbudgets`の権限](#configure-runner-api-permissions)を付与されたサービスアカウントが必要です。 |
...
## 一時停止ポッドでクラスター容量を事前にウォームアップ {#pre-warm-cluster-capacity-with-pause-pods}
...
Kubernetes executorを設定して、クラスター容量を事前にウォームアップする一時停止ポッドを維持できます。ジョブが開始されると、優先度の低い一時停止ポッドがプリエンプトされ、ジョブポッドは既存のノードで直ちにスケジュールされます。この設定により、クラスターオートスケーラーが新しいノードをプロビジョニングするのを待つジョブの起動レイテンシーが短縮されます。

### 一時停止ポッドのしくみ {#how-pause-pods-work}

1. Runnerは設定されたポリシーに基づいて一時停止ポッドの`Deployment`を作成します。
1. 一時停止ポッドは低い優先クラスを使用するため、優先度の高いジョブポッドがリソースを必要とするとKubernetesはそれらをプリエンプトします。
1. 一時停止ポッドがプリエンプトされると、ジョブポッドは直ちにその場所に引き継がれます。
1. `Deployment`は、プリエンプトされた一時停止ポッドを再作成し、クラスターオートスケーラーが新しいノードを追加するようにトリガーする可能性があります。

### 一時停止ポッドを設定する {#configure-pause-pods}

一時停止ポッドを有効にするには、`[runners.kubernetes.autoscaler]`セクションを`config.toml`に追加します:
...
### オートスケーラーの設定 {#autoscaler-settings}

| 設定 | 説明 |
|---------|-------------|
| `max_pause_pods` | 作成する一時停止ポッドの最大数。無制限の場合は`0`に設定します。 |
| `pause_pod_image` | 一時停止ポッドのイメージ。`registry.k8s.io/pause:3.10`がデフォルトです。 |
| `pause_pod_priority_class_name` | 一時停止ポッドの優先クラス。`gitlab-runner-idle-capacity`（優先度`-1`で自動作成）にデフォルトされます。指定すると、自動作成はスキップされます。 |

### プリエンプションの優先クラス {#priority-classes-for-preemption}

一時停止ポッドがジョブポッドによってプリエンプトされるには、より低い優先度である必要があります。既定では、Runnerは優先度`-1`の`PriorityClass`（名前：`gitlab-runner-idle-capacity`）を自動的に作成します。優先クラスのないポッドは優先度`0`を使用するため、ジョブポッドが一時停止ポッドをプリエンプトします。

代わりにカスタムの`PriorityClass`を使用するには、設定で指定します:
...
お使いのジョブポッドがカスタム優先クラスを使用する場合は、一時停止ポッド優先クラスよりも高い値を確保するようにしてください。

### ポリシー設定 {#policy-settings}

複数のポリシーを定義できます。現在時刻に基づく最後の一致するポリシーが使用されます。

| 設定 | 説明 |
|---------|-------------|
| `periods` | このポリシーがアクティブになる時期を定義するcron式の配列。`* * * * *`（常にアクティブ）にデフォルトされます。 |
| `timezone` | cron式を評価するためのタイムゾーン。システムの現地時刻にデフォルトされます。 |
| `idle_count` | 維持する一時停止ポッドの目標数。`0`がデフォルトです。 |
| `idle_time` | スケールダウンのクールダウン。希望する容量が減少すると、この待機時間の後に一時停止ポッドが削除されます。`scale_factor`を使用する場合のスラッシングを防止します。`5m`がデフォルトです。 |
| `scale_factor` | アクティブなジョブに基づいて一時停止ポッドをスケールします。`max(idle_count, active_jobs * scale_factor)`デフォルトは`0`（無効）です。 |
| `scale_factor_limit` | `scale_factor`を使用する場合の最大一時停止ポッド数。`0`（制限なし）にデフォルトされます。 |

### Cron構文 {#cron-syntax}

`periods`設定は、5つのフィールドを持つ標準のcron形式を使用します:
...
|--------|-------------|
| `* * * * *` | 常にアクティブ |
| `* 8-17 * * mon-fri` | 平日8:00～17:59 |
| `* 0-12 * * *` | 毎日深夜0:00～12:59 |

### 優先クラスを作成する {#create-the-priority-class}

一時停止ポッドには、ジョブポッドよりも低い優先度の優先クラスが必要です。一時停止ポッドを設定する前に、優先クラスを作成します:
...
### 必須のRBAC権限 {#required-rbac-permissions}

一時停止ポッドを使用するには、`Deployments`と`PriorityClasses`を管理するために、Runnerサービスアカウントに追加の権限を設定します:
...
> [!note]
> `PriorityClass`はクラスタースコープのリソースです。名前空間付きの`Role`と`RoleBinding`は、`scheduling.k8s.io/priorityclasses`権限を付与できません。代わりに`ClusterRole`と`ClusterRoleBinding`を使用してください。
...
> [!warning]
> これらのラベルは、Runner設定または`.gitlab-ci.yml`ファイルを通じて上書きできません。`runner.gitlab.com`ネームスペースでラベルを設定または変更する操作は無視され、デバッグメッセージとして記録されます。
...
> [!note]
> `FF_USE_POD_ACTIVE_DEADLINE_SECONDS`機能フラグを有効にし、`pod_termination_grace_period_seconds`をゼロ以外の値に設定すると、CI/CDジョブポッドは直ちに終了しません。ポッドの`terminationGracePeriods`により、有効期限が切れた場合にのみポッドが終了するようになります。

### ジョブポッドが削除されないようにする {#protect-job-pods-from-eviction}

{{< history >}}

- GitLab Runner 18.10で[導入](https://gitlab.com/gitlab-org/gitlab-runner/-/merge_requests/6331)されました。
...
これを有効にすると、ジョブポッドごとに`minAvailable: 1`の[`PodDisruptionBudget`](https://kubernetes.io/docs/tasks/run-application/configure-pdb/)が作成されます。このアクションにより、Kubernetesの削除APIが自主的な中断時にポッドを削除するのを防ぎます。
...
`PodDisruptionBudget`では:

- Kubernetesオーナー参照によってジョブポッドが削除されると、自動的に削除されます。
- ノード障害やメモリ不足による強制終了などの偶発的な中断からは保護しません。
- 追加のRBAC権限が必要です。詳細については、[Runner APIの権限を設定する](#configure-runner-api-permissions)を参照してください。

> [!warning]
> `PodDisruptionBudget`を有効にすると、ジョブが実行されている場合にノードドレインが停止する可能性があります。クラスターのアップグレード戦略が、潜在的なノードドレインの遅延を考慮するか、またはジョブタイムアウトを使用してジョブの実行時間を制限するようにしてください。
...
> [!warning]
> `runner.gitlab.com`ネームスペース内のラベルは読み取り専用です。GitLabは、これらのGitLab内部ラベルの追加、変更、または削除の試行操作をすべて無視します。
...
#### Mergeパッチ戦略 {#merge-patch-strategy}

`merge`パッチ戦略は、既存の`PodSpec`に[キー/バリュー置換](https://datatracker.ietf.org/doc/html/rfc7386)を適用します。この戦略を使用する場合、`config.toml`の`pod_spec`設定により、最終的な`PodSpec`オブジェクトの生成前に、このオブジェクトの値が**上書き**されます。値が完全に上書きされるので、このパッチ戦略を使用する際には十分に注意してください。
...
| `seccomp_profile.type` | 文字列 | いいえ | seccompプロファイルタイプ。有効な値：`RuntimeDefault`、`Localhost`、`Unconfined`。 |
| `seccomp_profile.localhost_profile` | 文字列 | いいえ | ノード上のseccompプロファイルへのパス。タイプが`Localhost`の場合に必須。 |
| `app_armor_profile.type` | 文字列 | いいえ | AppArmorプロファイルタイプ。有効な値：`RuntimeDefault`、`Localhost`、`Unconfined`。Kubernetes 1.30以降が必要です。 |
| `app_armor_profile.localhost_profile` | 文字列 | いいえ | ノード上のAppArmorプロファイルの名前。タイプが`Localhost`の場合に必須。 |
...
### seccompおよびAppArmorプロファイルを設定する {#set-seccomp-and-apparmor-profiles}

ネストされた`seccomp_profile`および`app_armor_profile`設定セクションを使用して、ビルドポッドの[seccomp](https://kubernetes.io/docs/tutorials/security/seccomp/)および[AppArmor](https://kubernetes.io/docs/tutorials/security/apparmor/)プロファイルを設定できます。

これらのフィールドは、非推奨のアノテーションベースのアプローチ（`container.apparmor.security.beta.kubernetes.io`および`seccomp.security.alpha.kubernetes.io`アノテーション）をネイティブのKubernetesAPIフィールドに置き換えます。

| フィールド | 最小Kubernetesバージョン |
|-------|---------------------------|
| `seccomp_profile` | 1.19 (GA) |
| `app_armor_profile` | 1.30 (GA) |

以下の例では、ルートレスイメージビルド（たとえばBuildKitを使用）を有効にするために、ビルドコンテナのseccompおよびAppArmorプロファイルが`Unconfined`に設定されています:
...
`seccomp_profile`と`app_armor_profile`セクションは、`pod_security_context`とすべてのコンテナのセキュリティコンテキスト（`build_container_security_context`、`helper_container_security_context`、`service_container_security_context`、`init_permissions_container_security_context`）の両方で利用できます。

`Localhost`タイプのプロファイルの場合は、プロファイルパスを指定します:
...
### コンテナユーザーとグループを設定する {#configure-container-user-and-group}

{{< history >}}

- GitLab Runner 18.4で、セキュリティコンテキストベースのユーザー設定のサポートが[導入](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/38894)されました。
...
Kubernetesでは、ユーザーIDとグループIDに数値を必要とします:
...
#### ユーザー設定およびグループ設定をオーバーライドする {#override-user-and-group-settings}

ポッドおよびコンテナ固有のセキュリティコンテキストを使用して、ユーザー設定およびグループ設定をオーバーライドします:
...
- ポッドセキュリティコンテキストは、固有の設定がないコンテナにデフォルト (1500:1500) を設定します
...
- rootユーザー/グループ（UID/GID 0）: ジョブ設定には常に明示的な許可リスト権限が必要
- 空の`allowed_users`: 非rootジョブユーザーはすべて許可
- 指定された`allowed_users`: リストされたジョブユーザーのみが許可
- 空の`allowed_groups`: 非rootジョブグループはすべて許可
- 指定された`allowed_groups`: リストされたジョブグループのみが許可
- セキュリティコンテキスト設定: 許可リストに対して検証されない（管理者オーバーライド）
...
| グループ形式                  | ユーザーフィールドではサポートなし        | 数値GID（`1000:1001`）                    |
| 管理者オーバーライド方法 | Runnerの`user`フィールド                | コンテナおよびポッドセキュリティコンテキスト          |
| 優先順位                    | Runner > ジョブ                       | コンテナコンテキスト > ポッドコンテキスト > ジョブ        |
| セキュリティ検証           | ユーザー名許可リスト                | 数値のUID/GID許可リスト                   |
| 管理者オーバーライド        | サポート対象                          | サポート対象（ポッドおよびコンテナレベル）         |
| ヘルパーコンテナユーザー         | ビルドコンテナと同じ            | 独自の`helper_container_security_context`を使用 |
| ポッドレベルのデフォルト            | 利用不可                      | `pod_security_context`                       |

#### ユーザー設定およびグループ設定のトラブルシューティングを行う {#troubleshoot-user-and-group-configuration}

##### エラー: `failed to parse UID`または`failed to parse GID` {#error-failed-to-parse-uid-or-failed-to-parse-gid}

- ユーザーIDが数値であることを確認します（`"user"`ではなく`"1000"`）
- 形式を確認します（ユーザーおよびグループに`"1000:1001"`）
- 負の値は許可されていません

##### エラー: `user "1000" is not in the allowed list` {#error-user-1000-is-not-in-the-allowed-list}

このエラーは、ジョブレベルのユーザー設定（`.gitlab-ci.yml`）の場合にのみ発生します。ユーザーをRunner設定の`allowed_users`に追加するか、`allowed_users`を削除して非rootジョブユーザーを許可します。セキュリティコンテキストとポッドセキュリティコンテキストのユーザーは、許可リストに対して検証されません。

##### エラー: `group "1001" is not in the allowed list` {#error-group-1001-is-not-in-the-allowed-list}

このエラーは、ジョブレベルのグループ設定（`.gitlab-ci.yml`）の場合にのみ発生します。グループをRunner設定の`allowed_groups`に追加するか、`allowed_groups`を削除して非rootジョブグループを許可します。セキュリティコンテキストとポッドセキュリティコンテキストのグループは、許可リストに対して検証されません。

##### エラー: `user "0" is not in the allowed list`（rootユーザーがブロックされています） {#error-user-0-is-not-in-the-allowed-list-root-user-blocked}

このエラーは、ジョブ設定（`.gitlab-ci.yml`）でルートが指定されている場合にのみ発生します。ジョブ設定のrootユーザー（UID 0）には明示的な権限が必要です。`"0"`を`allowed_users`に追加します。または、セキュリティコンテキストまたはポッドセキュリティコンテキストを使用してrootユーザーを設定します: `run_as_user = 0`（許可リスト検証をバイパスする）
...
ジョブに複数のサービスコンテナがある場合、各サービスコンテナに明示的なリソースリクエストと制限を設定できます。`.gitlab-ci.yml`で指定されているコンテナリソースを上書きするには、各サービスで変数属性を使用します。
...
`RuntimeClass`名を指定してもクラスターでこれを設定しない場合、または機能がサポートされていない場合は、executorはジョブを作成できません。
...
### ノードセレクターを上書きする {#overwrite-the-node-selector}
...
> [!note]
> `node_affinities`は、ビルドがどのオペレーティングシステムで実行されるべきかを決定するものではなく、`node_selectors`のみが決定します。詳細については、[オペレーティングシステム、アーキテクチャ、およびWindowsカーネルバージョン](#operating-system-architecture-and-windows-kernel-version)を参照してください。`config.toml`の設定例を次に示します。
...
> [!note]
> `match_label_keys`と`mismatch_label_keys`には、Kubernetes 1.31以降が必要です。以前のアルファまたはベータクラスターでは、`MatchLabelKeysInPodAffinity`機能ゲートを有効にします。機能ゲートをサポートしていないクラスターでは、APIサーバーはこれらのフィールドを無視し、制約は適用されません。
...
| `mount_propagation` | 文字列  | いいえ       | コンテナ間でマウントされたボリュームを共有します。詳細については、[マウントの伝播](https://kubernetes.io/docs/concepts/storage/volumes/#mount-propagation)を参照してください。 |
...
| `volume_attributes` | `map[string]string` | いいえ       | `csi`ボリュームの属性のキー/バリューペアマッピング。 |
| `sub_path`          | 文字列              | いいえ       | ルートの代わりにボリューム内の[サブパス](https://kubernetes.io/docs/concepts/storage/volumes/#using-subpath)をマウントします。 |
| `read_only`         | ブール値             | いいえ       | ボリュームを読み取り専用モードに設定します（デフォルトはfalseです）。 |
...
Kubernetes CIジョブのビルドディレクトリは、デフォルトでは一時的です。（`GIT_STRATEGY=fetch`を機能させるために）ジョブ間でGitクローンを永続化する場合は、ビルドフォルダーに対する永続ボリュームクレームをマウントする必要があります。複数のジョブを同時実行できるため、`ReadWriteMany`ボリュームを使用するか、同じRunner上で発生する可能性がある同時実行ジョブごとに1つのボリュームを用意する必要があります。後者の方がパフォーマンスの向上を見込めます。このような設定の例を以下に示します。
...
Docker 19.03以降では、TLSはデフォルトで有効になっていますが、クライアントに証明書をマップする必要があります。Docker-in-Dockerの非TLS接続を有効にするか、証明書をマウントできます。詳細については、[Docker executorでDocker-in-Dockerを使用する](https://docs.gitlab.com/ci/docker/using_docker_build/#use-the-docker-executor-with-docker-in-docker)を参照してください。
...
> [!note]
> GitLab 15.1以降では、`FF_KUBERNETES_HONOR_ENTRYPOINT`が設定されている場合、Dockerイメージで定義されたエントリポイントがKubernetesexecutorとともに使用されます。

コンテナのエントリポイントには、次の既知の問題があります。

- イメージのDockerfileでエントリポイントが定義されている場合、有効なShellを開く必要があります。このようにしないとジョブがハングします。
...
- `kube exec`を使ったKubernetes executorでは、GitLab RunnerはエントリポイントがShellを開くまで待機しませんでした（このセクションの前述の項目を参照）。
...
- `pods`と`configmaps`の場合
  ...
  - `watch`
  - `list`
- `pods/exec`と`pods/attach`の場合は`create`を使用します。
