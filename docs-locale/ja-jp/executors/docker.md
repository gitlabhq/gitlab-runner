...
The GitLab Runner emulated link behavior differs slightly from [legacyコンテナリンク](https://docs.docker.com/engine/network/links/):
...
> [!warning]
> イメージ名`**/*`によって、あらゆるイメージにデバイスが公開される可能性があります。
...
> [!warning]
> GitLab Runner 18.4以降では、Dockerボリュームベースおよびホストディレクトリベースの永続ストレージの両方で、動的ストレージのソースの命名（上記参照）が変更されました。18.4.0にアップグレードすると、GitLab Runnerは以前のRunnerバージョンのキャッシュされたデータを無視し、新しいDockerボリュームまたは新しいホストディレクトリを介して、オンデマンドで新しい動的ストレージを作成します。
>
...
> [!warning]
> 特権モードで実行されるコンテナにはセキュリティ上のリスクがあります。コンテナが特権モードで実行されている場合、コンテナセキュリティメカニズムを無効にし、ホストを特権エスカレーションに公開します。特権モードでコンテナを実行すると、コンテナのブレイクアウトが発生する可能性があります。詳細については、[ランタイム特権とLinux機能](https://docs.docker.com/engine/containers/run/#runtime-privilege-and-linux-capabilities)に関するDockerドキュメントを参照してください。
...
1. GitLab Runnerの`config.toml`ファイルを編集し、`[runners.docker]`セクションのhostエントリにソケット値を追加します。例: 
   ...
   > [!note]
   > 標準的なPodmanの使用には`privileged = false`を設定してください。ジョブ内で[Docker-in-Dockerサービス](#use-docker-in-docker-with-privileged-mode)を実行する必要がある場合にのみ、`privileged = true`を設定してください。
...
> [!note]
> `--docker-volumes`または`DOCKER_VOLUMES`環境変数を渡す際に、Runnerがソースディレクトリとして`c:\\cache`で登録されている場合、[既知のイシュー](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/4312)があります。
...
> [!note]
> Windowsコンテナの下位互換性により、Windows Server 2025 (24H2) は21H2 (Windows Server 2022) ヘルパーイメージを使用できます。
