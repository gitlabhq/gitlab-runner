...
GitLab Runnerはさまざまなexecutorを実装しています。これらのexecutorは、さまざまな環境でビルドを実行するために使用できます。以下のexecutorは、活発に機能開発が行われています:
...
> [!note]
> 一部の機能では、Runnerが[fleeting](../fleet_scaling/fleeting.md)を使用する必要があります。Docker Autoscalerとインスタンスのexecutorは、fleetingを使用します。GitLab Runnerの全機能を活用するには、これらのexecutorのいずれかに移行する必要があります。
...
これらのexecutorは、活発な機能開発は行われていません。これらは重要なセキュリティアップデートは受け取りますが、新機能は受け取りません。
...
> [!warning]
> この機能はGitLab 17.5で[非推奨](https://gitlab.com/gitlab-org/gitlab/-/issues/498268)になり、20.0での削除が予定されています。代わりに[GitLab Runner Autoscaler](../runner_autoscale/_index.md)を使用してください。
