...
> [!warning]
> デバッグロギングは重大なセキュリティリスクとなる可能性があります。出力には、ジョブで使用可能なすべての変数およびその他のシークレットの内容が含まれます。サードパーティにシークレットを送信する可能性のあるログ集計はすべて無効にする必要があります。マスクされた変数を使用すると、ジョブログ出力ではシークレットを保護できますが、コンテナログでは保護できません。
...
> [!warning]
> この操作は、[Shell executor](../executors/shell.md)を使用するRunnerでは実行しないでください。これは、`systemd`サービスを再定義し、すべてのジョブをrootとして実行するため、セキュリティリスクやファイル所有権の変更が生じ、特権のないアカウントに戻すことが難しくなります。これはセキュリティ上のリスクをもたらし、特権なしのアカウントに戻すことが困難になるファイル所有権の変更につながります。
...
この問題は、GitLab Runner RunnerワーカーがGitLabへの長期ポーリングリクエストでスタックし、他のジョブの即時処理を妨げる場合に発生します。これらの問題は、パフォーマンスのボトルネックから完全なデッドロックまで、設定によって異なります。この問題は、GitLab Workhorseの`apiCiLongPollingDuration`設定（デフォルト：）によって制御されるGitLab CI/CDの長期ポーリング機能に関連しています: 50秒）
...
Elasticsearchサービスコンテナの起動時に、次のようなエラーが発生する可能性があります:
