...
> [!warning]
> `shell`または`docker` executorを使用している場合は、グレースフルシャットダウンのために`killall`または`pkill`を**not**。これによりサブプロセスも強制終了されるため、シグナルが不適切に処理される可能性があります。ジョブを処理するメインプロセスでのみ使用してください。
...
このコマンドは、GitLab [Runners API](https://docs.gitlab.com/api/runners/)を使用して、GitLabにRunnerを登録します。
...
> [!note]
> RunnerはGitLab [API](https://docs.gitlab.com/api/runners/)を使用して直接登録できますが、設定は自動的に生成されません。
...
> [!note]
> ブールパラメータは、コマンドラインで`--key={true|false}`と共に渡す必要があります。
...
> [!warning]
> この操作は元に戻せません。この操作では設定ファイルが更新されます。このため、実行する前に`config.toml`のバックアップがあることを確認してください。
...
> [!note]
> RunnerはGitLab [API](https://docs.gitlab.com/api/runners/#delete-a-runner)で登録解除できますが、設定はユーザー向けには変更されません。
...
> [!note]
> 指定された名前のRunnerが複数ある場合は、最初の1つのみが削除されます。
...
- [**アクセスが拒否されました**](#access-denied-when-running-the-service-related-commands)
