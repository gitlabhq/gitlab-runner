...
| `FF_USE_NEW_BASH_EVAL_STRATEGY` | `false` | {{< icon name="check-circle" >}} 예 |  | 작동하지 않음(무작동)이며 지원 중단됨. Bash `eval` 호출은 서브셸에서 실행됩니다. 이 조치는 원래의 보호를 포함하며 취소 시 스크립트 본문 누출을 방지합니다. 자세한 내용은 [이슈 39005](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/39005)를 참조하세요. 이 플래그를 `false`로 설정한 경우의 동작인 레거시 파이프라인 호출 형식을 사용하려면 대신 새로운 `FF_USE_LEGACY_BASH_EVAL` 플래그를 사용하세요. 이 플래그는 향후 릴리스에서 제거됩니다. |
| `FF_USE_LEGACY_BASH_EVAL` | `false` | {{< icon name="dotted-circle" >}} 아니요 |  | `true`로 설정하면, Bash `eval` 호출은 지원 중단된 `FF_USE_NEW_BASH_EVAL_STRATEGY=false`와 동일한 동작인 레거시 파이프라인 호출 형식을 사용합니다. 기본값(`false`)은 `eval`를 갇힌 서브셸로 래핑하여 Bash 4.4 이전 버전 및 Bash 5.3 이상에서 취소 시 스크립트 본문 누출을 방지합니다. 자세한 내용은 [이슈 39005](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/39005)를 참조하세요. 기본 동작으로 인해 문제가 발생하면 이 플래그를 사용하세요. |
...
| `FF_USE_EXPONENTIAL_BACKOFF_STAGE_RETRY` | `true` | {{< icon name="dotted-circle" >}} 아니요 |  | 활성화되면, `GET_SOURCES_ATTEMPTS`, `ARTIFACT_DOWNLOAD_ATTEMPTS`, `RESTORE_CACHE_ATTEMPTS`, `EXECUTOR_JOB_SECTION_ATTEMPTS`, 및 `SECRETS_RETRIEVAL_ATTEMPTS`에 대한 재시도는 지수 백오프(5초 - 5분)를 사용합니다. |
...
| `FF_ENABLE_JOB_INPUTS_INTERPOLATION` | `true` | {{< icon name="dotted-circle" >}} 아니요 |  | 활성화되면 작업 입력이 보간됩니다. 자세한 내용은 [&17833](https://gitlab.com/groups/gitlab-org/-/work_items/17833)을 참조하세요. |
...
| `FF_USE_NATIVE_CONTAINER_STOP` | `false` | {{< icon name="dotted-circle" >}} 아니요 |  | 활성화되면, 작업 취소 시 빌드 컨테이너를 종료하기 위해 컨테이너 엔진의 네이티브 종료 메커니즘이 사용됩니다. SIGTERM은 PID 1에만 전달됩니다. PID 1은 프로세스 트리의 나머지 부분을 정상적으로 그리고 올바르게 종료해야 합니다. 이 플래그는 PID 1이 SIGTERM을 처리하고 종료를 하위 프로세스로 전파하는 것으로 알려진 경우에만 활성화하세요(신호를 전달하는 init 프로세스 또는 자체 종료를 조정하도록 설계된 애플리케이션처럼). 비활성화된 경우(기본값), 러너는 자체 정상 종료 메커니즘을 적용하며, 이는 하위 프로세스에 직접 신호를 보냅니다. 프로세스 트리에 자식 프로세스에 SIGTERM을 전파하지 않는 셸 또는 기타 프로세스가 포함되어 있을 때 직접 신호 전송이 필요합니다. |
