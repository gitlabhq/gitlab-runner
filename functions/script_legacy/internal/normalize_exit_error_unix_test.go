...
func TestNormalizeExitError_UnwrapsWaitDelayWhenExitCodeIsZero(t *testing.T) {
	...
	// though the foreground script exited cleanly. This is the
	// test-storybook teardown scenario that FF_SCRIPT_TO_STEP_MIGRATION
	// exposed.
...
func TestNormalizeExitError_PassesThroughFatalSignal(t *testing.T) {
	...
	// normalizeExitError must NOT swallow it.
	cmd := exec.Command("/bin/sh", "-c", "sleep 30")
	require.NoError(t, cmd.Start())
