...
type KubernetesConfig struct {
	...
	RequestRetryLimit                                 int                                `toml:"retry_limit,omitzero" json:"retry_limit" long:"retry-limit" env:"KUBERNETES_REQUEST_RETRY_LIMIT" default:"5" description:"The maximum number of attempts to communicate with Kubernetes API. The retry interval between each attempt is based on a backoff algorithm starting at 500 ms"`
	RequestRetryLimits                                []map[string]int                   `toml:"retry_limits" json:"retry_limits" long:"retry-limits" env:"KUBERNETES_RETRY_LIMITS" description:"How many times each request error or error group is to be retried"`
